...
实测在代码审查场景中，Sonnet 4.6 比 GPT-4 快 30%。`,
  ...
  {
    name: '直接相关：NavHub AI 导航网站更新',
    keyword: 'NavHub AI 导航',
    content: `NavHub AI 导航网站上线了全新的分类功能，新增了 50+ AI 工具推荐。
NavHub 团队表示，AI 导航将持续更新，帮助开发者快速找到最合适的 AI 工具。
新版本还支持用户收藏和评分功能。`,
    expectPass: true,
    expectKeywordMentioned: true,
    expectedRelevanceRange: [80, 100],
...
建议初学者从免费工具开始体验，逐步过渡到付费方案。`,
  ...
  {
    name: '间接相关：其他 AI 导航网站，不是 NavHub 的',
    keyword: 'NavHub AI 导航',
...
describe('Query Expansion（查询扩展）', () => {
  ...
  it('中文关键词能正确扩展', async () => {
    const result = await expandKeyword('NavHub AI 导航');
    expect(result).toContain('NavHub AI 导航');
    expect(result.some(r => r.includes('NavHub'))).toBe(true);
  });
});
